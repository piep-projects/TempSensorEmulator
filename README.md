...
| Taste | Kurz | Halten (3,5 s) |
|---|---|---|
| **BOOT** (GPIO 0) | −0,5 °C | WLAN-Portal öffnen |
| **KEY** (GPIO 14) | +0,5 °C | Deep Sleep |

Auto-Repeat ab 600 ms; stoppt 300 ms vor der 3,5-s-Schwelle.
...
Beim Start verbindet sich das Gerät **still** mit dem gespeicherten Netz (kein Blockieren).  
Das Captive Portal wird **auf Knopfdruck** gestartet:

1. **BOOT-Taste 3,5 s gedrückt halten** → Display zeigt WiFi-Setup-Screen
2. Handy mit **`CHA-Emulator`** (Passwort: `wolf1234`) verbinden
3. Browser öffnet sich **automatisch** → Heimnetz wählen, Passwort eingeben
4. Gerät verbindet sich und startet den Webserver
